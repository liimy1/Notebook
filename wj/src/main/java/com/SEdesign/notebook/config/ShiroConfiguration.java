package com.SEdesign.notebook.config;

import com.SEdesign.notebook.filter.URLPathMatchingFilter;
import com.SEdesign.notebook.realm.WJRealm;
...
public class ShiroConfiguration {
    ...
    public ShiroFilterFactoryBean shiroFilter(SecurityManager securityManager) {
        ...
        customizedFilter.put("url", getURLPathMatchingFilter()); // 自定义过滤器设置 2，命名url，需在设置过滤路径前

        // 对管理接口的访问启用自定义拦截（url 规则），即执行 URLPathMatchingFilter 中定义的过滤方法
        filterChainDefinitionMap.put("/api/authentication", "authc"); // 防鸡贼登录，只有认证了才能访问
    ...
    //不能使用bean注入否则与ShiroFilterFactoryBean冲突
